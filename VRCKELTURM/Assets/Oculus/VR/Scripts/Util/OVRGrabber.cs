        ...
        if (closestGrabbable != null)
        {
            ...
            if(m_grabbedObj.snapPosition)
            {
                ...
                if (m_grabbedObj.snapOffset)
                {
                    ...
                    {
                        snapOffset.x = -snapOffset.x + 0.05f;
                        snapOffset.z -= 0.05f;
                    }
                    m_grabbedObjectPosOff = snapOffset;
                }
            ...
            if (m_grabbedObj.snapOrientation)
            {
                ...
                if (m_grabbedObj.snapOffset)
                {
                    if (m_controller == OVRInput.Controller.LTouch)
                    {
                        ...
                        var localRotation = m_grabbedObj.snapOffset.localRotation;
                        //m_grabbedObjectRotOff = Quaternion.Inverse(localRotation) * Quaternion.Euler(1f, 180f, 1f);
                        Vector3 rot = localRotation.eulerAngles;
                        rot = new Vector3(rot.x,rot.y,rot.z+180);
                        m_grabbedObjectRotOff = Quaternion.Euler(rot);
                    }
                    else
                    {
                        var localRotation = m_grabbedObj.snapOffset.localRotation;
